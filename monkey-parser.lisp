...
(defmethod make-special-grammar ((kind (eql '^)) sexp) ;; extract is just sugar around build 
  ;; The use of CDDR below is a partial attempt to allow a production
  ;; to return NIL. But it doesn't quite work because there's some
  ;; higher level that thinks a NIL result is the same as no result.
  ;; Probably need to bind result to a special no-result value and
  ;; check for that instead.
  (make-instance 'build-grammar
		 :grammar (make-grammar (second sexp))
		 :form `(setf result ,(if (cddr sexp) (third sexp) 'last-match))))
...
(defmethod emit-production-function ((production grammar-production) (parser parser))
  (let ((start-sym (gensym)))
    (with-slots (name variables grammar) production
      `(,name (&key save-p)
	      (let (,name ,@variables (,start-sym index))
		...
		(symbol-macrolet ((result ,name))
		  (and ,(code grammar parser)
			 ...
			 (when save-p
			   (setf last-match
				 (or ,name (subseq input ,start-sym index))))
			 t))))))))
